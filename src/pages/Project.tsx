...
export default function PoolPage() {
  ...
  const openPanelsDialog = () => { loadPanels(); setPanelsOpen(true); };
  const closePanelsDialog = () => setPanelsOpen(false);
  ...
  // Dialog içi Upload Excel (Catalog) için ref
  ...
  return (
    <Box sx={{ minHeight: "100vh", background: "radial-gradient(circle at top right, #1A237E, #000000)", color: "#FFFFFF", display: "flex", flexDirection: "column" }}>
      ...
      <Box sx={{ p: 2, borderTop: "1px solid rgba(255,255,255,0.1)", display: "flex", justifyContent: "flex-end", gap: 2, flexWrap: "wrap" }}>
        ...
        {/* View Panels (Assign to Panel entry point) */}
        <PrimaryButton onClick={openPanelsDialog}>Assign to Panel</PrimaryButton>
        ...
        <DialogContent dividers>
          ...
          >
            ...
            <FormControl size="small" sx={{ minWidth: 180 }}>
              ...
              <Select label="Signal Type" value={signalFilter} onChange={(e) => setSignalFilter(String(e.target.value))}>
                <MenuItem value="">(All)</MenuItem>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap">
              {selectedForRow(selectorRowKey).map((it) => (
                <Chip
                  key={`sel-${it.id}`}
                  size="small"
                  label={`${it.brand || ""} ${it.name || it.code || it.model || ""}`.trim() || it.id}
                  onDelete={() => {
                    const prev = selectedForRow(selectorRowKey);
      ...
      {/* Panels Dialog (View Only + Delete) */}
      <Dialog open={panelsOpen} onClose={closePanelsDialog} fullWidth maxWidth="lg">
        <DialogTitle sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          Panels {projectFilter ? `– ${projectFilter}` : ""}
          <IconButton onClick={closePanelsDialog} size="small"><CloseIcon /></IconButton>
        </DialogTitle>
        <DialogContent dividers>
          <Box sx={{ border: "1px solid #eee", borderRadius: 1, overflow: "hidden" }}>
            <table style={{
        ...
        <DialogActions sx={{ p: 2 }}>
          <PrimaryButton onClick={closePanelsDialog}>Close</PrimaryButton>
        </DialogActions>
      </Dialog>
